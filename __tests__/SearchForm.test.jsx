...
describe("SearchForm", () => {
  ...
  it("calls onSearch with the correct arguments when a dropdown value changes", async () => {
    ...
    expect(mockOnSearch).toHaveBeenCalledWith("area", "Pisgah National Forest");
    const difficultyDropdown = screen.getByRole("combobox", { name: /difficulty rating/i });
  ...
  it("renders all area options correctly", () => {
    render(<SearchForm />);
    const areaDropdown = screen.getByRole("combobox", { name: /area name/i });
    const options = Array.from(areaDropdown.options).map((option) => option.text);
